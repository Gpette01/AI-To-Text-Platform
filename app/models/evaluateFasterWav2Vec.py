...
# Load the Distilled Wav2Vec2 model and processor
distil_model_id = "facebook/wav2vec2-large-xlsr-53-distilled"
distil_model = Wav2Vec2ForCTC.from_pretrained(distil_model_id).to("cuda" if torch.cuda.is_available() else "cpu")
distil_processor = Wav2Vec2Processor.from_pretrained(distil_model_id)
...
# Function to transcribe audio with DistilWav2Vec2
def transcribe_with_distil(audio_path):
    audio_input, _ = librosa.load(audio_path, sr=16000)
    input_values = distil_processor(audio_input, return_tensors="pt", sampling_rate=16000).input_values
    input_values = input_values.to(distil_model.device)
    with torch.no_grad():
        logits = distil_model(input_values).logits
    predicted_ids = torch.argmax(logits, dim=-1)
    transcription = distil_processor.batch_decode(predicted_ids)[0]
...
for filename in sorted(os.listdir(audio_directory)):
    if filename.endswith(".wav"):
        ...
        # Transcribe the audio using DistilWav2Vec2
        start_time = time.time()
        distil_hypothesis = transcribe_with_distil(temp_audio_path)
        elapsed_time = time.time() - start_time

        # Evaluate transcription
        metrics = evaluate_transcription(reference_text, distil_hypothesis)
        metrics.update({
            "Model": "DistilWav2Vec2",
            "Filename": filename,
            "Elapsed_Time": elapsed_time,
            "Hypothesis": distil_hypothesis,
            "Reference": reference_text,
        })
        results.append(metrics)

        print(f"Processed {filename} with DistilWav2Vec2:")
        print(f"  - Elapsed time: {elapsed_time:.2f}s")
        print(f"  - Hypothesis: {distil_hypothesis}")
        print(f"  - Reference: {reference_text}")
        print(f"  - WER: {metrics['WER']:.2f}, MER: {metrics['MER']:.2f}, WIL: {metrics['WIL']:.2f}")

# Save results to a CSV file
results_df = pd.DataFrame(results)
results_df.to_csv("transcription_evaluation_distilwav2vec2.csv", index=False)
